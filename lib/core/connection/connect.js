...
function connect(options, cancellationToken, callback) {
  ...
  if (options.host == 'embedded') {
    global.USE_MONGO_EMBEDDED = true;
  }
  console.log("Use MongoEmbedded", global.USE_MONGO_EMBEDDED);
  if (global.USE_MONGO_EMBEDDED) {
    const mongoEmbedded = require('../../embedded/embedded');
    const socket = new Proxy({}, {
      get(target, prop, receiver) {
        if (prop === 'isConnected') return () => true;
        if (prop === 'ismaster') return () => true;
        //if (prop === 'compressor') return () => 'snappy';
        if (prop === 'workItems') return {
          push() {
          }
        }
        return () => null;
    ...
    mongoEmbedded.connect(options.path);
